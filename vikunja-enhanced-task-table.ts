...
(function () {
     ...
     */
    function extractTaskIdFromElement(element: HTMLElement): number {
        const row = element.closest('tr');
        return extractTaskIdFromRow(row);
    }

    /** Retrieves the textual label for the Done column from the UI. */
    function getDoneColumnLabelText(): string {
    ...
    async function fetchCurrentUser(): Promise<User> {
        if (!cachedUser) {
            cachedUser = await new Promise<User>((resolve) => {
                GM_xmlhttpRequest({
                    method: 'GET',
                    url: '/api/v1/user',
                    headers: {
     ...
     */
    function getVisibleColumnPosition(columnIndex: number): number {
     ...
     */
    async function setupEditableTitleCell(cell: HTMLTableCellElement): Promise<void> {
    ...
    function focusContentEditableAtEnd(element: HTMLElement): void {
     ...
     */
    async function fetchTasks(ids: number[]): Promise<Task[]> {
     ...
     */
    async function fetchTasksBatchFromApi(taskIds: number[]): Promise<Task[]> {
        const results: Task[] = [];
     ...
     */
    async function addDateColumnFeature(
        columnIndex: number,
     ...
     */
    async function updateAssigneeSearchButtonVisibility(
        menu: HTMLDivElement,
        cell: HTMLTableCellElement
    ): Promise<void> {
     ...
     */
    function handleLabelSearch(input: HTMLInputElement, menu: HTMLDivElement): void {
        const query = input.value.trim();
        const resultsContainer = menu.querySelector<HTMLDivElement>('.search-results');
        if (!resultsContainer) return;
     ...
     */
    function createTaskAddFormElement(): HTMLFormElement {
     ...
     */
    function updateTaskAddFormVisibility(): void {
     ...
     */
    function clearCachedTaskData(): void {
     ...
     */
    async function reorderTaskRows(rows: NodeListOf<HTMLTableRowElement>): Promise<void> {
